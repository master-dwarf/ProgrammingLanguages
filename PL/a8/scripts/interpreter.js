    ...
    if (A.isIntExp(exp)) {
      ...
      console.log( JSON.stringify(
        ...
        E.lookupReference(
          ...
          var block = A.getLetrecExp(exp);
          var bArgs = A.getFnExpParams(A.getAppExpFn(A.getLetrecExp(exp)));
          var recfunc = A.getLetrecExpArgs(exp);
          var clo = evalExp(recfunc[0],envir);
          envir = E.update(envir, bArgs, [clo]);
          for(i = 0; i < 666; i++) {
            clo = E.createClo(E.getCloParams(clo), E.getCloBody(clo), envir);
            envir = E.update(envir, bArgs, [clo]);
          }
          return callByValue(block,envir);
        }
