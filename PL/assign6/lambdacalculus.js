// Assignment 6  parser and interpreter for lambda-calculus Expressions
...
var us = require("./underscore/underscore-min");
...
function parse_and_prettify (str) {
    str = str.replace(/^\s+|\s+$/g, '');
    console.log(require('util').inspect(parser.parse(str), false ,null, true));
    return require('util').inspect(parser.parse(str), false ,null, true);
    //    return require('util').puts(require('util').inspect(parser.parse(str), false ,20, true));
};

exports.interpret = eval_and_prettify;
